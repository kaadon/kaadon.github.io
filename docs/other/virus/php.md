...
> 1)  查找含有该代码的文件
> ```shell
> grep -rn --include="*.php" '$b11st=0;' .
> 
> rg -n -uuu -a --glob '*.php' --fixed-strings '$l0ader' . | awk -F: '{print $1 ":" $2}'
> ```
...
>     if grep -qE "\\\$b[0-9]+st=0;" "$file" && grep -qE "\\\$b[0-9]+ed=0;" "$file"; then
>         echo "Cleaning $file"
>         sed -i.bak -E '/\$b[0-9]+st=0;/,/\$b[0-9]+ed=0;/d' "$file"
>     fi
> done
> 
> ```
...
> ```shell
> 
> nohup bash -c 'find . -type f -name "*.php" | while read -r file; do if grep -qE "\$b[0-9]+st=0;" "$file" && grep -qE "\$b[0-9]+ed=0;" "$file"; then echo "Cleaning $file"; sed -i.bak -E "/\$b[0-9]+st=0;/,/\$b[0-9]+ed=0;/d" "$file"; fi; done' > clean_php.log 2>&1 &
> 
> ```
>
>
...
>
> ```shell
>  apt install ripgrep #(需要先安装)
> 
> nohup bash -c 'tmp=$(mktemp); rg -l -0 -uuu -a --glob "*.php" --glob "!*.bak" '\''\$l0ader|\$b[0-9]+st=0;|\$b[0-9]+ed=0;'\'' . > "$tmp"; [ -s "$tmp" ] && xargs -0 perl -0pi.bak -e '"'"'s/;\$b[0-9]+st=0;.*?;\$b[0-9]+ed=0;//sg; s/\$l0ader\s*=\s*function\s*\(\$check\)\s*\{.*?\$b[0-9]+ed=0;//sg; s/;\$b[0-9]+st=0;//g; s/;\$b[0-9]+ed=0;//g; s/\$b[0-9]+ed=0;//g'"'"' < "$tmp"; rm -f "$tmp"; rg -n -uuu -a --glob "*.php" --glob "!*.bak" '\''\$l0ader|\$b[0-9]+st=0;|\$b[0-9]+ed=0;'\'' .' > clean_php.log 2>&1 &
> 
> tail -f clean_php.log
>
> ```
> 
